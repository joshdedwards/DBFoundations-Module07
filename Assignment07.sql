...
-- 2017-01-01,JEdwards,Created File
--**************************************************************************--
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_JEdwards')
	 Begin 
	  Alter Database [Assignment07DB_JEdwards] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_JEdwards;
	 End
	Create Database Assignment07DB_JEdwards;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_JEdwards;
...
As
	Select Top 1000000
	P.ProductName
	,[InventoryDate] = DateName(MM, I.InventoryDate) + ', ' + DateName(YY,InventoryDate)
	,[InventoryCount] = I.[Count]
	From vProducts as P
		Inner Join vInventories as I
...
As
	Select Top 1000000
		...
		,CountVsPreviousCountKPI] = IsNull(Case
			When Inventory > [PreviousMonthCount] then 1
			When Inventory = [PreviousMonthCount] then 0
			When Inventory < [PreviousMonthCount] then -1
			End, 0)
...
Create
View fProductInventoriesWithPreviousMonthCountsWithKPIs (@KPIValue int)
Returns Table
AS
	Return Select
		ProductName
